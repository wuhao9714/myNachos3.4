...
void Machine::TLBswap(int pn,int way){
    int i;
    for(i=0;i<TLBSize;i++){
        if(!tlb[i].valid){
            tlb[i]=currentThread->space->getIPTE(pn);//getPTE(pn);//getPageTable()[pn];
    ...
    i=0;
    for(int j=1;j<TLBSize;j++){
        if(tlb[i].lastused>tlb[j].lastused)
            i=j;
    }
	}
    tlb[i]=currentThread->space->getIPTE(pn);//getPTE(pn);//getPageTable()[pn];
...
void Machine::printIPT(){
	for(int i=0;i<NumPhysPages;i++){
		if(invertedPageTable[i].valid==TRUE){
			printf("ppn:%d\tvpn:%d\ttid:%d\n",invertedPageTable[i].physicalPage
				,invertedPageTable[i].virtualPage,invertedPageTable[i].tid);
		}
		else
			printf("ppn:%d is free for now\n", i);
	}
}